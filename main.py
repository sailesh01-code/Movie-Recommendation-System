...
# =========================
load_dotenv()
# Check for both typos in API key env variable
...
if not TMDB_API_KEY:
    ...
    raise RuntimeError("TMDB_API_KEY missing. Put it in .env as TMDB_API_KEY=0b40fdd43cf8573abfc61f9b2544cc1d")
...
def tfidf_recommend_by_overview(
    overview: str, top_n: int = 10
) -> List[Tuple[str, float]]:
    """
    Returns list of (title, score) from local df using cosine similarity on an arbitrary overview text.
    Useful when the searched movie does not exist in the local dataset.
    """
    ...
    # Vectorize the text dynamically
    qv = tfidf_obj.transform([overview])
    scores = (tfidf_matrix @ qv.T).toarray().ravel()
    ...
    for i in order:
        ...
        if score > 0.0:  # Only recommend if there's any similarity
...
async def search_bundle(
    ...
    try:
        ...
        # fallback to overview based similarity
        try:
            recs = tfidf_recommend_by_overview(details.overview or query, top_n=tfidf_top_n)
        except Exception:
            recs = []
